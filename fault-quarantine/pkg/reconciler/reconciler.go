...
func (r *Reconciler) applyRuleLabelsForEvent(
	...
	for _, eval := range ruleSetEvals {
		result, err := eval.Evaluate(event)
		if err != nil {
			return fmt.Errorf("evaluate ruleset %q for session labels: %w", eval.GetName(), err)
		}
...
func (r *Reconciler) handleManualUncordon(nodeName string) error {
	...
	labelAnnotationsToRemove, appliedLabelKeys, err := appliedLabelCleanupParams(annotations)
	if err != nil {
	...
	labelsToRemove = append(labelsToRemove, appliedLabelKeys...)
